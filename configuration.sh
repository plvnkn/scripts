...
zone2=$(dialog --clear --title "ZONE" --no-tags --menu "Select your area" 60 10 60 $(ls -l /usr/share/zoneinfo/$continent | awk '{ print NR" "$0 }') 2>&1 1>&3 2>&3 3>&-)
area=$(ls -l /usr/share/zoneinfo | grep '^d' | awk '{ print $9 }' | sed -n ${zone2}p)
